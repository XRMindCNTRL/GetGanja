...
if ([string]::IsNullOrEmpty($resourceGroup) -or [string]::IsNullOrEmpty($webappName)) {
    Write-Host "❌ Azure configuration not found. Please run setup-production-clean.ps1 first." -ForegroundColor Red
    exit 1
}
...
az webapp up --resource-group $resourceGroup --name $webappName --runtime "NODE:18-lts" --os-type Linux --location "southafricanorth"
...
function Deploy-FrontendApp {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        az staticwebapp appsettings set --name $staticAppName --resource-group $resourceGroup --setting REACT_APP_API_URL="https://$webappUrl" --setting REACT_APP_NODE_ENV="production"
    } else {
        Write-Host "Creating new Static Web App..." -ForegroundColor Cyan
        az staticwebapp create --name $staticAppName --resource-group $resourceGroup --location "southafricanorth" --source . --output-location "build" --login-with-github false --sku "Free"
    }
